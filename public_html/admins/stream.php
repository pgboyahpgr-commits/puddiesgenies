...
<nav class="max-w-5xl mx-auto px-4 py-4 flex items-center gap-3 flex-wrap">
  ...
  <span class="text-gray-400">/ Stream Control</span>
</nav>
<main class="max-w-5xl mx-auto px-4 pb-12">
  <h1 class="text-3xl font-bold mb-6" style="color:#2D3436;">📺 Live Stream Setup</h1>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    ...
    <div class="bg-white/80 backdrop-blur rounded-2xl p-5 shadow border border-gray-100">
      <h3 class="font-bold mb-3">Stream URL</h3>
      <form method="POST" class="flex flex-col gap-3">
        <input type="hidden" name="csrf_token" value="<?=$csrfToken?>" />
        <input type="hidden" name="action" value="update_url" />
        <input type="text" name="video_url" id="videoUrl" value="<?=htmlspecialchars($videoUrl)?>" placeholder="Paste YouTube URL or any embed URL..." class="w-full px-4 py-3 rounded-full border-2 border-gray-200 outline-none focus:border-[#4ECDC4] text-sm" />
        <button type="submit" class="px-6 py-2 rounded-full font-bold text-white" style="background:#4ECDC4;">Save URL</button>
      </form>
      <p class="text-xs text-gray-400 mt-2">Supports YouTube, Vimeo, or any direct embed URL.</p>
    ...
    <div class="relative" style="padding-top:56.25%;background:#000;border-radius:16px;overflow:hidden;">
      ...
      <div class="absolute inset-0 flex items-center justify-center text-gray-500">
        <div class="text-center">
          ...
          <p class="text-xs text-gray-600 mt-2">Set a URL and turn the stream ON to preview</p>
        </div>
      </div>
      <?php endif; ?>
    </div>
  ...
  <div class="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mt-4">
    <p class="text-sm text-yellow-800 font-bold mb-1">📌 How to set up</p>
    <ol class="text-sm text-yellow-700 list-decimal list-inside space-y-1 mt-2">
      <li>Go to <a href="https://www.youtube.com" target="_blank" class="underline">YouTube</a> and start a live stream or pick a video</li>
      <li>Copy the URL from your browser address bar</li>
      <li>Paste it in the <strong>Stream URL</strong> field above and click <strong>Save URL</strong></li>
      <li>Click <strong>Turn On</strong> — customers will see the stream on the <a href="/kitchen.php" target="_blank" class="underline">Live Kitchen page</a></li>
    </ol>
  </div>
</main>
</body>
