...
<main class="max-w-6xl mx-auto px-4 pb-12">
  <h1 class="text-3xl font-bold mb-4" style="color:#2D3436;">💬 Talk Inbox</h1>

  <?php if (!empty($callRequests)): ?>
  <div class="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mb-4">
    <h3 class="font-bold text-sm mb-2">📞 Pending Call Requests</h3>
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
    <div class="bg-white/80 backdrop-blur rounded-2xl p-4 shadow border border-gray-100">
      <h3 class="font-bold text-sm mb-3">Tables</h3>
      <?php
        $guestFile = $msgDir . 'table_0.json';
        $guestCount = file_exists($guestFile) ? count(json_decode(@file_get_contents($guestFile), true) ?: []) : 0;
      ?>
      <a href="?table=0" class="flex items-center justify-between px-3 py-2 rounded-xl mb-1 <?=$selectedTable===0?'bg-[#FF6B6B] text-white':'hover:bg-gray-100'?> no-underline">
        <span>👤 Guest Talk</span>
      ...
      ?>
      <a href="?table=<?=$t['number']?>" class="flex items-center justify-between px-3 py-2 rounded-xl mb-1 <?=$selectedTable===$t['number']?'bg-[#FF6B6B] text-white':'hover:bg-gray-100'?> no-underline">
    ...
    <div class="md:col-span-3 bg-white/80 backdrop-blur rounded-2xl shadow border border-gray-100 overflow-hidden flex flex-col" style="height:500px;">
      ...
      <div class="flex-1 overflow-y-auto p-4" id="adminChatMessages" style="display:flex;flex-direction:column;">
        ...
        <?php endforeach; ?>
        <?php if (empty($messages)): ?><div class="text-center text-gray-400 py-8 flex-1 flex items-center justify-center" id="emptyMsg">No messages yet</div><?php endif; ?>
      </div>
      <div class="flex gap-2 p-3 border-t border-gray-200">
        <input type="text" id="adminChatInput" placeholder="Type reply..." class="flex-1 px-4 py-3 rounded-full border-2 border-gray-200 outline-none focus:border-[#FF6B6B] text-sm" />
        <button id="adminSendBtn" class="px-6 py-3 rounded-full font-bold text-white" style="background:#FF6B6B;">Send</button>
      </div>
      <?php else: ?>
      <div class="flex items-center justify-center h-full text-gray-400">Select a table to view messages</div>
      <?php endif; ?>
    </div>
  </div>
</main>
...
.chat-msg.admin { background:#e9e9e9; color:#2D3436; align-self:flex-start; border-bottom-left-radius:4px; }
.chat-msg.table { background:#4ECDC4; color:#fff; align-self:flex-end; border-bottom-right-radius:4px; margin-left:auto; }
