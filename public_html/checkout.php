...
<main class="max-w-3xl mx-auto px-4 pb-12">
  ...
  <div class="text-center mb-8 pt-4">
    <h1 class="text-5xl font-bold" style="color:var(--text);"><span style="color:#f68e9a;" data-translate>Checkout</span></h1>
    <?php if ($tableNum > 0): ?>
  ...
  <div id="cartItems" class="mb-6">
    <div class="text-center py-8 text-gray-400" data-translate>Loading cart...</div>
  </div>
  ...
  <div class="glass-card p-4 mb-6" id="priceBreakdown">
    <div id="subtotalRow" class="flex justify-between text-sm text-gray-500 mb-1">
      ...
      <span id="subtotalAmount" data-translate>₹0</span>
    </div>
    <div id="taxRow" class="flex justify-between text-sm text-gray-500 mb-1">
      <span data-translate>Tax (5% GST)</span>
      <span id="taxAmount" data-translate>₹0</span>
    </div>
    <div class="border-t border-gray-200 my-2"></div>
    <div class="flex justify-between items-center">
      <span class="font-bold text-lg" data-translate>Total</span>
      <span class="font-bold text-2xl" style="color:#f68e9a;" id="cartTotal" data-translate>₹0</span>
    </div>
  </div>

  <div class="glass-card p-4 mb-6">
    <h3 class="font-bold mb-2" data-translate>📝 Custom Instructions</h3>
    <textarea id="specialInstructions" placeholder="Any special requests for your order... (e.g., extra spicy, no onions)" class="w-full p-3 rounded-xl border-2 border-gray-200 bg-white/80 outline-none focus:border-[#538bdf] transition text-sm min-h-[80px]"></textarea>
  </div>

  <div class="flex flex-col gap-3">
    <button onclick="placeOrder('online')" id="payOnlineBtn" class="btn-bouncy py-4 text-lg w-full" style="background:#538bdf;color:#fff;">
...
window.updateCartDisplay = function() {
  ...
  if (items.length === 0) {
    container.innerHTML = '<div class="glass-card p-6 text-center"><div class="text-4xl mb-2" data-translate>🛒</div><p class="text-gray-400" data-translate>Your cart is empty</p><a href="/menu.php" class="inline-block mt-3 btn-bouncy px-6 py-2 no-underline" style="background:#f68e9a;color:#fff;" data-translate>Browse Menu</a></div>';
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    html += '<div class="font-semibold text-sm" data-translate>' + escapeHtml(item.name) + '</div>';
    html += '<div class="text-xs text-gray-400" data-translate>₹' + (item.price || 0) + ' × ' + (item.qty || 1) + '</div>';
    html += '</div>';
    html += '<div class="flex items-center gap-3">';
    html += '<div class="flex items-center border-2 border-gray-200 rounded-full overflow-hidden">';
    html += '<button class="cart-qty-btn" data-qty-id="' + id + '" data-qty-delta="-1" style="width:30px;height:30px;border:none;background:transparent;font-size:1.1rem;font-weight:700;cursor:pointer;" data-translate>−</button>';
    html += '<span class="font-bold w-6 text-center text-sm" data-translate>' + (item.qty || 1) + '</span>';
    ...
    html += '</div>';
    html += '<div class="font-bold text-sm w-16 text-right" style="color:#f68e9a;" data-translate>₹' + itemTotal + '</div>';
    html += '</div></div>';
  }
...
function placeOrder(mode) {
  ...
  fetch('/api/save-order.php', {
    ...
    body: JSON.stringify({ items: items, total: total, subtotal: subtotal, tax: tax, instructions: instructions, payment: mode, table: table, customer_name: window.USER_NAME || '' })
  })
