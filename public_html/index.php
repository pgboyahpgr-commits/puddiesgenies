...
$urlToken = $_GET['t'] ?? '';
if ($urlToken) {
  $tf = __DIR__ . '/data/tables.json';
  if (file_exists($tf)) {
    $ts = @json_decode(@file_get_contents($tf), true);
    foreach ($ts['tables'] ?? [] as $t) {
      if (($t['token'] ?? '') === $urlToken) {
...
<style>
  ...
  .blob-1 { width: 400px; height: 400px; background: #f68e9a; top: -100px; right: -100px; }
  .blob-2 { width: 300px; height: 300px; background: #538bdf; bottom: -50px; left: -80px; }
  .blob-3 { width: 250px; height: 250px; background: #f7b6bf; top: 40%; left: 60%; }
  ...
  .portal-card {
    ...
    min-height: 160px; color: var(--text);
  }
...
<div class="max-w-lg mx-auto px-4 py-8 text-center">
  <div class="mb-8">
    <div class="text-6xl mb-2 float-anim" data-translate>🍛</div>
    <h1 class="text-5xl font-bold" style="color:var(--text);">Smak<span style="color:#f68e9a;">AI</span></h1>
    <p class="text-gray-500 mt-1" data-translate>Smart Restaurant Experience</p>
    <?php if ($tableNum > 0): ?>
    <div class="inline-block mt-3 px-5 py-2 rounded-full font-bold text-sm" style="background:#538bdf;color:#fff;">
      🪑 Table #<?= $tableNum ?>
    </div>
    <?php endif; ?>
  </div>
  ...
  <div class="grid grid-cols-2 gap-4">
    <a href="/menu.php" class="portal-card">
      <div class="icon" data-translate>📋</div>
      <div class="label" data-translate>Menu</div>
      <div class="sub" data-translate>Browse all dishes</div>
    </a>
    <a href="/akinator.php" class="portal-card">
      <div class="icon" data-translate>🤖</div>
      <div class="label" data-translate>AI Waiter</div>
      <div class="sub" data-translate>Let me guess!</div>
    </a>
    <a href="/food-vlogs.php" class="portal-card">
      <div class="icon" data-translate>🎬</div>
      <div class="label" data-translate>Food Vlogs</div>
      <div class="sub" data-translate>Watch & discover</div>
    </a>
    <a href="/kitchen.php" class="portal-card">
      <div class="icon" data-translate>📺</div>
      <div class="label" data-translate>Live Kitchen</div>
      <div class="sub" data-translate>See food being made</div>
    </a>
    <a href="/talk.php" class="portal-card">
      <div class="icon" data-translate>💬</div>
      <div class="label" data-translate>Talk to Manager</div>
      <div class="sub" data-translate>Chat or call us</div>
    </a>
    <a href="/invoice.php" class="portal-card">
      <div class="icon" data-translate>🧾</div>
      <div class="label" data-translate>My Bill</div>
      <div class="sub" data-translate>View your invoice</div>
    </a>
  </div>

  <div class="mt-8 text-xs text-gray-400" data-translate>
